...
public class StatsUtil2
{
  static final int dumpStatsInterval = 60;
  static final int dumpLinkStatsInterval = 300;
  
  /**
   * Używana do liczenia średnich prędkości z pominięciem pojazdów czekających
   * w kolejce na czerwonym świetle
  ...
  public static void dumpLinkStats(final City city,
            final PrintWriter statWriter, LinkStat linkStat, LinkStat linkRidingStat) {
    ...
    for (Iterator<Link> i = city.linkIterator(); i.hasNext();) {
      ...
      for (int j = 0; j < avgVelocities.size(); j++) {
        ...
        statWriter.print(String.format("\t<period begin='%d' end='%d' avg_velocity='%.4f' avg_riding_velocity='%.4f'>\n",
                begin,end,avgVelocity, avgRidingVelocity));
        period++;
      }
   ...
   * @param linkRidingStat średnia prędkość statystyki dla wszystkich pojazdów oprócz tych stojących na czerwnoym świetle
   */
  public static void collectLinkStats(final City city,
      final CarInfoIView carInfoView,
      final BlockIView blockView,
      int turn,
      LinkStat linkStat, LinkStat linkRidingStat)
  {
    for (Iterator<Link> i = city.linkIterator(); i.hasNext();) {
      Link link = i.next();
        ...
        while (infoBackwardCursor != null && infoBackwardCursor.isValid()) {
            try {
                ...
                // sprawdzam czy ten pojazd też mogę ignorować
                    ...
                    if (currentPos != (lastPos-1) || currentVelocity > 0) {
                        skipingMode = false;
                    }
                }
  ...
  public static void dumpStats(final City city,
      final MiniStatEView statView,
      final int turn,
      final PrintWriter writer)
  {
    ...
    statWriter.println( "===========" );
    statWriter.printf(
        "%6s %6s %6s %15s %15s %15s %15s\n", "from", "to", "count", "avg. duration",
        "<-std dev.", "avg. velocity", "<-[kph]" );
    for (Iterator<Gateway> i1 = city.gatewayIterator(); i1.hasNext();) {
      ...
      for (Iterator<Gateway> i2 = city.gatewayIterator(); i2.hasNext();) {
        ...
        if ( rs != null && rs.getTravelCount() > 0 ) {
          ...
          statWriter.printf(
              "%6s %6s %6d %15.1f %15.1f %15.2f %15.1f\n", g1.getId(), g2.getId(), rs
                  .getTravelCount(), rs.getAvgDuration(), rs.getStdDevDuration(), v, vkph );
        }
      }
    }
    ...
    for (Iterator<Link> i = city.linkIterator(); i.hasNext();) {
      ...
      if ( linkExt.getDriveCount() > 0 ) {
        ...
        statWriter.printf( "%6s %6s %6d %15.1f %15.1f % 15.2f %15.1f\n", link
            .getBeginning().getId(), link.getEnd().getId(), linkExt.getDriveCount(), linkExt
            .getAvgDuration(), linkExt.getStdDevDuration(), v, vkph );
      }
    }
